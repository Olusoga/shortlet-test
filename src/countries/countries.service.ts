...
export class CountriesService {
  ...
  private async getCachedData(cacheKey: string, fetchFunction: () => Promise<any>): Promise<any> {
    ...
    await this.redisService.set(cacheKey, JSON.stringify(data), 3600); // Cache for 1 hour
    return data;
  }
    ...
    return this.getCachedData(cacheKey, async () => {
      try {
        ...
        const sortedCountries = sortBy
          ? filteredCountries.sort((a, b) => (a[sortBy] < b[sortBy] ? -1 : a[sortBy] > b[sortBy] ? 1 : 0))
          : filteredCountries;

        // Pagination
        const paginatedCountries = sortedCountries.slice((page - 1) * limit, page * limit);

        this.logger.log(`Fetched and processed countries data with query: ${JSON.stringify(query)}`);
        return {
        ...
        throw new HttpException('Error fetching countries data', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    });
  }
  ...
  async getCountryByName(name: string): Promise<CountryDetailsDto> {
    ...
    return this.getCachedData(cacheKey, async () => {
      try {
        ...
        this.logger.error(`Error fetching country details for ${name}`, error.message);
        if (error.response && error.response.status === 404) {
          throw new HttpException('Country not found', HttpStatus.NOT_FOUND);
        }
        throw new HttpException('Error fetching country details', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    });
  }
  ...
  private async fetchCountriesByRegion(region: string): Promise<any[]> {
    ...
    return this.getCachedData(cacheKey, async () => {
      ...
      this.logger.log(`Fetched ${countries.length} countries for region ${region}`);
      return countries;
    });
  }
  ...
  async fetchStatistics(): Promise<any> {
    ...
    return this.getCachedData(cacheKey, async () => {
      ...
      const mostSpokenLanguage = Object.keys(languageSpeakers).reduce(
        (acc, language) => languageSpeakers[language] > acc.speakers
